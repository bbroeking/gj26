...
func _render() -> void:
	...
	for rid in st.get("recipes", []):
		...
		# Wrap each recipe in a parchment card so the list reads as a carved
		# shelf of options, not a flat Godot widget stack.
		var card := PanelContainer.new()
		var card_sb := WyrdUi.chip_stylebox(
			WyrdUi.KIT_PLATE if not locked else Color(0.86, 0.81, 0.69))
		...
		card_sb.content_margin_bottom = 6.0
		if locked:
			card_sb.border_color = Color(WyrdUi.KIT_EDGE, 0.30)
