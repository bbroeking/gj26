...
# Art pass: Caveat hand font + tier-coloured ink outlines so each number reads
# as a storybook glyph rather than a flat engine label.

const _FONT_HAND := preload("res://assets/fonts/Caveat.ttf")
...
# Ink outline per tier — sepia / dark amber / deep violet — each tier stamps
# a distinct hand-painted glyph into the world.
const TIER_OUTLINE := {
	"normal": Color(0.22, 0.14, 0.08),
	"crit":   Color(0.45, 0.24, 0.04),
	"super":  Color(0.18, 0.06, 0.28),
}
const LIFETIME := 0.62
...
func setup(amount: int, tier: String = "normal") -> void:
	...
	font_size = 80
	outline_size = 16
	outline_modulate = TIER_OUTLINE.get(tier, TIER_OUTLINE["normal"])
	text = str(amount)
...
# number so it doesn't compete with the actual hit numbers.
func setup_apply(label: String, color: Color) -> void:
	...
	font_size = 56
	outline_size = 12
	outline_modulate = Color(0.12, 0.08, 0.16)   # faint violet-ink shadow
	var ang := randf() * TAU
