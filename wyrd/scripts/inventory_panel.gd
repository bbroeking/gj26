...
func _draw_slots() -> void:
	...
	for name in SLOT_OFFSET:
		...
		# Carved socket well — kit primitive gives 3-level stepped inner shadow
		# + light bottom lip so each slot reads as carved into the doll column.
		...
		if it != null:
			...
			# Empty slot — category name in small-caps header font, ghosted.
			draw_string(hdr2, r.position + Vector2(0.0, r.size.y * 0.5 + 7.0),
				String(name).capitalize(), HORIZONTAL_ALIGNMENT_CENTER,
				r.size.x, 14, Color(0.50, 0.42, 0.32, 0.70))
