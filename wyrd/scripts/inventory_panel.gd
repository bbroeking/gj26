...
func _draw_tabs(win: Rect2) -> void:
	...
	for i in TABS.size():
		var r := _tab_rect(i)
		var active := i == _tab
		# Carved parchment plate — active = warm cream, inactive = muted parchment.
		draw_rect(r, Color(0.95, 0.91, 0.80) if active else Color(0.85, 0.78, 0.64))
		# Top light bevel — the tab face catches the page's warm ambient light.
		var bevel_a := 0.38 if active else 0.20
		draw_rect(Rect2(r.position + Vector2(2.0, 1.0), Vector2(r.size.x - 4.0, 1.5)),
			Color(1.0, 1.0, 0.93, bevel_a))
		...
		# Storybook icon disc well on the left of each tab. Textures are
		# preloaded in _ready (avoids the load-inside-_draw white-rect gotcha).
		var icon_tex: Texture2D = _cached_tex(String(TAB_ICONS[i]))
		if icon_tex != null:
			var dc := r.position + Vector2(20.0, r.size.y * 0.5)
			WyrdUi.draw_round_well(self, dc, 10.0,
				Color(0.88, 0.81, 0.67) if active else Color(0.78, 0.71, 0.58))
			draw_texture_rect(icon_tex,
				Rect2(dc - Vector2(6.5, 6.5), Vector2(13.0, 13.0)), false,
				WyrdUi.INK if active else Color(WyrdUi.INK_MID, 0.85))
			draw_string(font, r.position + Vector2(34.0, 22.0), String(TABS[i]),
				HORIZONTAL_ALIGNMENT_CENTER, r.size.x - 38.0, 15,
