...
func _draw_trades_tab(win: Rect2, font: Font, scroll: float, view: Rect2) -> void:
	...
	for i in TRADE_ROWS.size():
		...
		if _span_visible(y - 14.0, y + 58.0, scroll, view):
			...
			WyrdUi.draw_well(self, bar, Color(0.80, 0.72, 0.58))
			if frac > 0.0:
				var fill_r := Rect2(bar.position + Vector2(2, 2),
					Vector2(maxf(0.0, (bar.size.x - 4.0) * frac), bar.size.y - 4.0))
				draw_rect(fill_r, (row.color as Color).lightened(0.12))
		...
		for p in perks:
			...
			if _span_visible(cardy - CARD_GAP, cardy + CARD_H + CARD_GAP, scroll, view):
				...
				if ok:
					WyrdUi.draw_parchment_grain(self, cr, pl * 31)
				else:
					draw_rect(cr.grow(-1.5), Color(0.80, 0.74, 0.62, 0.35))
