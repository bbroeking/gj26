...
func _draw_trades_tab(win: Rect2, font: Font, scroll: float, view: Rect2) -> void:
	...
	for i in TRADE_ROWS.size():
		...
		if _span_visible(y - 14.0, y + 58.0, scroll, view):
			...
			if frac > 0.0:
				var fw := maxf(0.0, (bar.size.x - 6.0) * frac)
				var fr := Rect2(bar.position + Vector2(3, 3), Vector2(fw, bar.size.y - 6.0))
				draw_rect(fr, (row.color as Color))
				draw_rect(Rect2(fr.position, Vector2(fw, 1.5)), Color(1.0, 1.0, 0.93, 0.40))
		...
		for p in perks:
			...
			if _span_visible(cardy - CARD_GAP, cardy + CARD_H + CARD_GAP, scroll, view):
				...
				WyrdUi.draw_list_row(self, cr,
					WyrdUi.SAGE if ok else WyrdUi.INK_MID)
				if not ok:
					draw_rect(cr.grow(-1.5), Color(0.0, 0.0, 0.0, 0.10))
