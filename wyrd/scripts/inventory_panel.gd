...
func _draw_trades_tab(win: Rect2, font: Font, scroll: float, view: Rect2) -> void:
	...
	for i in TRADE_ROWS.size():
		...
		if _span_visible(sy - 18.0, sy + 14.0, scroll, view):
			...
			# Flourish rule under the header — the same ◆ divider used across panels.
			WyrdUi.draw_flourish(self, Vector2(cx + (w - 78.0) * 0.5, sy + 10.0), 140.0)
		# Cards march down a spine in the left gutter; the disc lights when earned.
		# Cards now use draw_list_row (bevel + KIT_PLATE face + left accent stripe)
		# matching the carved-card language of vendor/loadout/satchel panels.
		var lx := x + 18.0
		var card_x := x + 42.0
		var card_w := w - 50.0
		var cardy := sy + 18.0
		...
		for p in perks:
			...
			if _span_visible(cardy - CARD_GAP, cardy + CARD_H + CARD_GAP, scroll, view):
				...
				# Carved card: sage stripe when earned, dim stripe + faded wash when locked.
				WyrdUi.draw_list_row(self, cr, WyrdUi.SAGE if ok else WyrdUi.INK_MID)
				if not ok:
					# Muted wash dims the card face so locked perks read as out-of-reach.
					draw_rect(cr.grow(-1.5), Color(0.80, 0.74, 0.62, 0.45))
				else:
					# Parchment grain on earned cards — per-card deterministic seed.
					WyrdUi.draw_parchment_grain(self, cr, perk_idx * 17 + 5)
				...
				if ok:
					# Gold inner ring on earned nodes — the burnished "lit" read.
					draw_arc(dc, 7.0, 0.0, TAU, 16, Color(WyrdUi.GOLD, 0.75), 1.5, true)
