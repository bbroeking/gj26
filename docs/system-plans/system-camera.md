...
`wyrd/scripts/camera_rig.gd` and `wyrd/scenes/CameraRig.tscn` together implement the FATE-style telephoto rig: 35° FOV set on the `Camera3D` node (CameraRig.tscn:9), standalone pivot (not parented to the player) that smoothly follows the player via `lerp` at `FOLLOW_SPEED 6.0` (camera_rig.gd:17), lead-targeting that nudges the pivot up to `MAX_LEAD 1.8` units ahead of velocity (lines 21–22), right-mouse-drag orbit + `Z`/`C` keyboard yaw (line 57–58 — moved from Q/E to free `E` for interact), scroll-wheel zoom between 8–22 m (lines 34–35), and a `look_at()` placement on a sphere around the pivot so the framing holds at any pitch (lines 132–139). Screen-shake is implemented as a jitter-after-look_at decay (`_shake_amt`, line 50; `shake()` method at line 154), keyed by tier through `HitFeedback.SHAKE_BY_TIER` (`hit_feedback.gd:27–29`: normal 0.06, crit 0.15, super 0.30). Wall occlusion runs a multi-step raycast that fades `GeometryInstance3D.transparency` on any "wall"-group GLB between camera and player (lines 162–201). Co-op local-authority follow is handled: `_acquire_player` checks `is_multiplayer_authority()` and retries every 0.2s if NetGame hasn't spawned the local player yet (lines 64–72).
