...
const PITCH_MIN := 30.0            # degrees above the horizon
const PITCH_MAX := 80.0            # kept off 90° so look_at never degenerates
const PITCH_DEFAULT := 55.0        # Diablo-style ARPG 3/4 angle
const ZOOM_MIN := 8.0
const ZOOM_MAX := 28.0
const ZOOM_STEP := 2.0
const ZOOM_DEFAULT := 17.0         # spec 29 — pull camera back so the frame
                                   # holds multiple rooms (Diablo framing), not
                                   # a single room cropping its edges
...
func _ready() -> void:
	...
	_bind("cam_yaw_left", KEY_Q)
	_bind("cam_yaw_right", KEY_E)
