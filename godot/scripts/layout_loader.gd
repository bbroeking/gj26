...
const HEDGEMOTHER_MODEL := "res://models/hedgemother_v2.glb"

const ENEMY_MODELS := [
	"res://models/enemy_skeleton_v1.glb",
	"res://models/enemy_rat_v1.glb",
	"res://models/enemy_ghost_v1.glb",
	"res://models/enemy_hedge_sprite_v1.glb",
]
# Per-model visual scale — calibrated against the reference via
# Calibration.tscn (spec 16). Same order as ENEMY_MODELS. The GLBs are
# authored at wildly different native sizes; these normalise them.
const ENEMY_SCALES := [1.45, 0.55, 2.00, 2.00]   # skeleton, rat, ghost, sprite
const BOSS_SCALE := 3.75
const ENEMY_COUNT := 6
...
func _ready() -> void:
	...
	_build_decor(layout)
	_build_boss(layout.bossRoom, layout.grid)
	var enemy_n := _build_enemies(layout)
	_position_player(layout.entry)

	# Dev: when DEBUG_SHOT, self-capture a viewport screenshot once the scene
	# settles — lets the camera framing be reviewed without a native-window
...
func _get_layout() -> Dictionary:
	if USE_PROCGEN:
		return DungeonGenScript.generate(-1)
...
func _build_boss(boss_room, grid: Array) -> void:
	if boss_room == null:
		return
	var cx: int = int(boss_room.x) + int(int(boss_room.w) / 2.0)
	var cy: int = int(boss_room.y) + int(int(boss_room.h) / 2.0)
	var model := _resolve_model(HEDGEMOTHER_MODEL)
	var packed: PackedScene = load(model)
	if packed == null:
		return
	var inst: Node3D = packed.instantiate()
	# Calibrated boss scale (spec 16) — Hedgemother towers at ~2× a mob.
	inst.scale = Vector3.ONE * BOSS_SCALE
	_unmetal(inst)
	# No tint — the rigged Hedgemother GLB carries its own 2048² texture.
	var boss := _spawn_character(inst, cx, cy, 0.9, 3.0, BOSS_HP, BossScript)
...
func _build_enemies(layout: Dictionary) -> int:
	...
	for ri in rooms.size():
		...
		var elite_kind: int = ei % ENEMY_MODELS.size()
		...
		for k in n:
			...
			if k == elite_idx:
				...
				_spawn_enemy(ei, t.x, t.y, role, depth)
				ei += 1
	return ei
...
func _spawn_enemy(ei: int, tx: int, ty: int, role: String = "combat",
		depth: int = 0, forced_idx: int = -1) -> CharacterBody3D:
	# Spec 32 — `forced_idx` lets the elite's retinue use the SAME kind as
	# the elite leader (Brambled skeleton → skeleton retinue). Default -1
	# preserves the spec 27b round-robin behavior for non-elite spawns.
	var idx := forced_idx if forced_idx >= 0 else (ei % ENEMY_MODELS.size())
	var model: String = _resolve_model(ENEMY_MODELS[idx])
	var packed: PackedScene = load(model)
	if packed == null:
		return null
	var inst: Node3D = packed.instantiate()
	inst.scale = Vector3.ONE * float(ENEMY_SCALES[idx])
	_unmetal(inst)
	# Rigged enemies carry real GLB textures; only the rat (idx 1) gets a tint.
	if idx == 1:
		_tint(inst, ENEMY_TINTS[idx])
	var body := _spawn_character(inst, tx, ty, 0.4, 1.4, ENEMY_HP)
...
# Solid stylized tint per character — the Meshy GLB textures don't read in
# Godot (white-blob look); flat toon colors suit the low-poly style and are
# deterministic. Keyed to ENEMY_MODELS order: skeleton, rat, ghost, sprite.
const ENEMY_TINTS := [
	Color(0.82, 0.80, 0.72),   # skeleton — bone
	Color(0.42, 0.32, 0.24),   # rat — brown
	Color(0.55, 0.82, 0.88),   # ghost — pale cyan
	Color(0.38, 0.52, 0.28),   # hedge-sprite — moss
]
