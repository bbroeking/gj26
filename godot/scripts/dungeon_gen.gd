...
static func generate(seed_value: int = -1) -> Dictionary:
	...
	for attempt in MAX_ATTEMPTS:
		...
		var layout := _generate_one(rng)
		var score: Dictionary = DungeonScoreScript.score_layout(layout, _score_cfg())
		layout["score"] = score
...
# ---- one layout ----
static func _generate_one(rng: RandomNumberGenerator) -> Dictionary:
	var grid := []
	for y in GRID:
		var row := []
		for x in GRID:
	...
	for _i in ROOM_TRIES:
		...
		var rx := rng.randi_range(1, GRID - w - 2)
		var ry := rng.randi_range(1, GRID - h - 2)
		var overlaps := false
		for r in rooms:
...
# ---- loop edges: ~LOOP_EDGE_FRACTION of the non-MST Delaunay edges ----
static func _loop_edges(all_edges: Array, mst_edges: Array, rng: RandomNumberGenerator) -> Array:
	var mst_keys := {}
	for e in mst_edges:
	...
	for i in range(extras.size() - 1, 0, -1):
		...
		extras[i] = extras[j]
		extras[j] = tmp
	var take := int(floor(extras.size() * LOOP_EDGE_FRACTION))
